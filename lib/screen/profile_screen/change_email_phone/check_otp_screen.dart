...
class _CheckOtpScreenState extends State<CheckOtpScreen> {
  ...
  checkOtp(sum) async {
    ...
    final response = await http.post(
      Uri.parse('$ipcon/check_otp/$user_id'),
  ...
  change_email() async {
    ...
    final response = await http.post(
      ...
      body: jsonEncode(<String, String>{"email": widget.email.toString()}),
    );
    if (response.statusCode == 200) {
      var data = json.decode(response.body);
      print(data);
      if (data == "update success") {
        showDialog(
            ...
            builder: (context) => SimpleDialog(
                  title: Center(
                      child: Text("Update Email Succes",
                          style: TextStyle(
  ...
  sendOtp() async {
    ...
    if (response.statusCode == 200) {
      ...
      if (data == "send email success") {
        showDialog(
            context: context,
            builder: (context) => BuildShow("Send Email Again Success"));
      }
    }
  }
